...
def test_successful_read():
    ...
    with patch("builtins.open", mock_open(read_data=mock_data)):
        data = get_transactions_dictionary("path_to_file.json")
        assert data == [
            {
                "id": 441945886,
                "state": "EXECUTED",
...
def test_file_not_found():
    ...
    with patch("builtins.open", side_effect=FileNotFoundError):
        data = get_transactions_dictionary("non_path_file.json")
        assert data == []
...
def test_json_decode_error():
    ...
    with patch("builtins.open", mock_open(read_data=mock_data)):
        data = get_transactions_dictionary("uncorrect_file.json")
