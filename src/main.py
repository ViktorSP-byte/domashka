...
def format_file():
    ...
    while True:
        try:
            ...
            logger.error(f"ошибка при выборе формата файла {e}")
    ...
    for transaction in transactions:
        ...
        if "from" in transaction and isinstance(transaction["from"], str):
            if "счет" in transaction["from"].lower():
                number_account = mask_account_card(transaction["from"])
                name_account = filter_alpha_data(transaction["from"])
                digital_mask_account = get_mask_account(number_account)
                masked_transaction["from"] = f"{name_account} {digital_mask_account}"
            else:
                number_card = mask_account_card(transaction["from"])
                name_card = filter_alpha_data(transaction["from"])
                digital_mask_card = get_mask_card_number(number_card)
